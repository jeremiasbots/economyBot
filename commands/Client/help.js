  ...
  async execute(message, args) {
    ...
    if (args.length === 1) {
      ...
      if (command) {
        ...
        commandInfo += translate(message.author.id, "help.command-description", { command_description: command.description });
    ...
    client.commands.forEach((command) => {
      const category = command.category || translate(message.author.id, "help.other");
    ...
    categories.forEach((commands, category) => {
      helpMessage += translate(message.author.id, "help.category", { category });
